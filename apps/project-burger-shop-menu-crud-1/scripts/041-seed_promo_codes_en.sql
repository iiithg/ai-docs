-- Seed data for promo codes (English version)
...
values
  ...
  ('SAVE20', '20% off orders over ¥50', 'percentage', 20, 5000, 2000, null, true, now(), now() + interval '60 days'),
  ('STUDENT15', 'Student exclusive 15% off', 'percentage', 15, 1000, 1500, 200, true, now(), now() + interval '90 days'),

  -- Active fixed amount discounts
  ('FIRST5', '¥5 off first order', 'fixed_amount', 500, 2000, null, 50, true, now(), now() + interval '14 days'),
  ('SAVE10', '¥10 off orders over ¥30', 'fixed_amount', 1000, 3000, null, null, true, now(), now() + interval '45 days'),
  ('LUNCH3', '¥3 off lunch time special', 'fixed_amount', 300, 1500, null, 100, true, now(), now() + interval '7 days'),
  ...
  ('VIP20', 'VIP exclusive 20% off', 'percentage', 20, 5000, 5000, 5, true, now(), now() + interval '365 days')
