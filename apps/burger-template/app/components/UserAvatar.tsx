...
export interface UserInfo {
  ...
  isEmoji?: boolean; // Whether it's an emoji avatar
}
    ...
    <div className="relative user-avatar-container">
      ...
      >
        {userInfo.isEmoji !== false ? (
          // Emoji avatar
          <span className="text-lg filter drop-shadow-sm">{userInfo.avatar}</span>
        ) : (
          // Image avatar
          <img
            src={userInfo.avatar}
            ...
            onError={(e) => {
              // Show default emoji when image fails to load
        ...
        {userInfo.isEmoji === false && (
          <span className="text-lg hidden filter drop-shadow-sm">👤</span>
        )}
      </button>
      ...
      {showProfile && (
        <div className="absolute right-0 top-12 w-80 bg-white rounded-xl shadow-2xl border border-gray-100 z-50 overflow-hidden animate-in slide-in-from-top-2 duration-200">
          ...
          <div className="bg-gradient-to-r from-burger-yellow to-burger-red p-5 text-white">
            <div className="flex items-center gap-4">
              <div className="w-16 h-16 rounded-full bg-white bg-opacity-20 flex items-center justify-center border-2 border-white shadow-md">
                {userInfo.isEmoji !== false ? (
                  <span className="text-3xl filter drop-shadow-sm">{userInfo.avatar}</span>
                ) : (
                  <img
                    src={userInfo.avatar}
                    alt={userInfo.name}
                ...
                {userInfo.isEmoji === false && (
                  <span className="text-3xl hidden filter drop-shadow-sm">👤</span>
                )}
              </div>
              <div>
          ...
          <button
            ...
            className="absolute top-3 right-3 w-8 h-8 rounded-full bg-white bg-opacity-20 hover:bg-opacity-30 flex items-center justify-center text-white transition-all duration-200 hover:scale-110"
            aria-label="Close"
          >
            ✕
          </button>
